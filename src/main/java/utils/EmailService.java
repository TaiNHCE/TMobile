...
public class EmailService {
    ...
    public static boolean sendOTPEmail(String recipientEmail, String type) throws UnsupportedEncodingException {
        ...
        OTPManager otpManager = otpMap.get(recipientEmail);
        if (otpManager != null) {
            if (!otpManager.isExpired()) {
                if (!otpManager.canResend()) {
                    System.err.println("\u274C Đã vượt quá số lần gửi lại OTP (tối đa 3 lần).");
                    return false;
                }
                otpManager.incrementResend();
            } else {
                otpManager = new OTPManager(generateVerificationCode(), 5);
                otpMap.put(recipientEmail, otpManager);
            }
        } else {
            otpManager = new OTPManager(generateVerificationCode(), 5);
            otpMap.put(recipientEmail, otpManager);
        ...
        int code = otpManager.getOtpCode();
